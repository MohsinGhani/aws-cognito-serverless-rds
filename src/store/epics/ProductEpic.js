...
export default class ProductEpic {
  static reverseGeoCoding = action$ =>
    action$.ofType(REVERSE_GEOCODING).switchMap(({ payload }) => {
      return HttpService.get(
        ...
        .switchMap(response => {
          if (response.status === 200) {
            ...
            return Observable.of(
              ProductAction.reverseGeoCodingSuccess(
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.likeProductFailure({ error: err.message })
          );
        });
    });

  static likeProduct = action$ =>
    action$.ofType(LIKE_DISLIKE_PRODUCT).switchMap(({ payload }) => {
  ...
  static sendFeedback = action$ =>
    action$.ofType(SEND_FEEDBACK).switchMap(({ payload }) => {
      return HttpService.post(path.SEND_FEEDBACK, payload)
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.sendFeedbackFailure({ error: err.message })
          );
        });
    });

  static search = action$ =>
    action$.ofType(SEARCH).switchMap(({ payload }) => {
      return HttpService.get(`${path.SEARCH}/${payload.query}`)
        .switchMap(({ response }) => {
          if (response.status === 200) {
            return Observable.of(
              ProductAction.searchSuccess({
                data: response.data,
                query: payload.query
              })
            );
  ...
  static getCategories = action$ =>
    action$.ofType(GET_CATEGORIES).switchMap(({ payload }) => {
      return HttpService.get(path.GET_CATEGORIES)
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.getCategoriesFailure({ error: err.message })
          );
        });
    });

  static saveProduct = action$ =>
    action$.ofType(SAVE_PRODUCT).switchMap(({ payload }) => {
      return HttpService.post(path.SAVE_PRODUCT, payload)
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.saveProductFailure({ error: err.message })
          );
        });
    });

  static doCommentOnProduct = action$ =>
    action$.ofType(DO_COMMENT_ON_PRODUCT).switchMap(({ payload }) => {
      return HttpService.post(path.DO_COMMENT_ON_PRODUCT, payload)
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.doCommentOnProductFailure({ error: err.message })
          );
        });
    });

  static actionOnComment = action$ =>
    action$.ofType(ACTION_ON_COMMENT).switchMap(({ payload }) => {
      return HttpService.post(path.ACTION_ON_COMMENT, payload)
        .switchMap(({ response }) => {
          if (response.status === 200) {
            return Observable.of(
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.actionOnCommentFailure({ error: err.message })
          );
        });
    });
  ...
  static getProducts = action$ =>
    action$.ofType(GET_PRODUCTS).switchMap(({ payload }) => {
      return HttpService.get(path.GET_PRODUCTS)
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.getProductsFailure({ error: err.message })
          );
        });
    });

  static getProductById = action$ =>
    action$.ofType(GET_PRODUCT_BY_ID).switchMap(({ payload }) => {
      return HttpService.get(`${path.GET_PRODUCT}/${payload.product_id}`)
        ...
        .catch(err => {
          return Observable.of(
            ProductAction.getProductByIdFailure({ error: err.message })
          );
        });
    });
}
